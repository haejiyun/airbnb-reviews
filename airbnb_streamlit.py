...
with SentimentTab: ################################################# Sentiment Analysis
    ...
    if not arrondissement or selected_min == selected_max : #Warning message when no data is selected
        ...
        with col2: #On the second column
            ...
            st.write("") #Blank line
            wordcloud = plt.figure(facecolor = 'none', figsize=(5,4)) #Set graph background transparent
            ax1 = wordcloud.add_axes([0, -0.5, 1, 1]) #Configure wordcloud position for positive comments
