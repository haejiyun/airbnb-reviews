...
with col1: #On the first column
    ...
    st.dataframe(data = df[['listing_id','latitude','longitude']].head(10).rename(columns ={"listing_id":"id"}), hide_index=True) #Show the dataset
with col2: #On the second column
    st.write("*reviews.csv*") #Show the dataset name
    st.table(data = df[['listing_id','date','comments']].head(2)) #Show the dataset
...
st.write("") #Blank line
st.markdown('<span style="font-size: 24px;">Translation</span>', unsafe_allow_html=True) #Subheader
st.write('''
...
st.write("") #Blank line
st.markdown('<span style="font-size: 24px;">Sentiment Score</span>', unsafe_allow_html=True) #Subheader
st.write('''
...
st.write("") #Blank line
st.markdown('<span style="font-size: 24px;">Multi-Label Classification</span>', unsafe_allow_html=True) #Subheader
st.write('''
